function [error] = evalConfidenceIntervalsWithConstrainedMetConcs2(config_script_NN, config_script_NN2, rand_i, rxn_rank)
...
rxn_i = rxns_sorted_by_accuracy(nan_start-rxn_rank);

if isfile("results_confidence_intervals_metConstraints_"+string(rxn_i)+"_"+string(rand_i)+".mat")
    return
end
...
[ci_lb, ci_ub, x_lb, x_ub] = calc_confidence_interval_constrained_mets( ...
    ...
    rxn_nsfs, rxn_min, rxn_max, Chi2_lim, minX(1:num_nspfactors), rangeX(1:num_nspfactors), x_selectors_per_mid);

save("results_confidence_intervals_metConstraints_"+string(rxn_i)+"_"+string(rand_i), "ci_lb", "ci_ub", "x_ub", "x_lb");
