function [standardized_nsfs, standardized_met_concs, resnorm, residual] = ...
    fit_NN_to_NSFs_and_known_mets_Chi2v2(fitOptions, sample_MIDs_to_estimate, sds_per_MID, nsf_base, met_base, ...
                                NNs_per_MID_and_tp,mu_per_MID_and_tp,sigma_per_MID_and_tp, x_selectors_per_mid)

    num_of_nsfs = size(nsf_base,1);
    num_of_mets = size(met_base,1);
    
    test_Y_flat = reshape(sample_MIDs_to_estimate',[],1);
    sds_per_Y_flat = reshape(sds_per_MID',[],1);
    
    degrees_of_freedom = max(length(test_Y_flat)/2, length(test_Y_flat) - (num_of_mets + num_of_nsfs));

    fun = @(x)(double(forwardSigmoidnet2v2(NNs_per_MID_and_tp,mu_per_MID_and_tp,sigma_per_MID_and_tp, x, x_selectors_per_mid)-test_Y_flat)./sds_per_Y_flat);
    num_vars = num_of_nsfs + num_of_mets; % length(mu_per_MID_and_tp{1,1});
    lb = zeros(num_vars,1);
